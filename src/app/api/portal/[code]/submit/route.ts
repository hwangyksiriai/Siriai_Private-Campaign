...
export async function POST(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (body.type === "settlement_reuse") {
    const updated = await submitSettlementFromExistingProfile(applicationId, influencer.id);
  ...
  if (body.type === "settlement_new") {
    ...
    const updated = await submitSettlementNew(applicationId, influencer.id, {
      realName,
      phone,
      bankName,
      bankAccount,
