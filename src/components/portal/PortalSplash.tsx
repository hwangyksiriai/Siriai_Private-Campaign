export default function PortalSplash() {
  return (
    <section className="relative flex h-screen min-h-[640px] flex-col items-center justify-center overflow-hidden bg-white">
      <div className="flex flex-col items-stretch gap-1 select-none">
        <span className="block text-center leading-none tracking-[-0.02em] text-black/13 [animation:textFlow_.8s_cubic-bezier(.22,.6,.36,1)_.2s_both] [font-size:clamp(56px,8vw,96px)]">
          <span style={{ fontFamily: "var(--font-logo-serif)" }} className="font-normal">
            Siri
          </span>
          <span style={{ fontFamily: "var(--font-body)" }} className="font-black">
            ai
          </span>
        </span>
      ...
      <div className="absolute bottom-12 flex flex-col items-center gap-2.5">
        <span className="text-[9.5px] tracking-[.22em] text-black/20 uppercase">Scroll</span>
        <div className="flex flex-col items-center gap-[5px]">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="h-1 w-1 rounded-full bg-black/18 [animation:bounce_1.6s_ease_infinite]"
