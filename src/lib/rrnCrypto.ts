 ...
 * siriai-admin의 lib/rrnCrypto.ts와 동일한 방식 — RRN_SECRET을 공유하면
 * 나중에 실제 admin DB로 그대로 연동할 수 있어요.
 */
...
function key(): Buffer {
  ...
  const secret = process.env.RRN_SECRET || process.env.AUTH_SECRET || "siriai-rrn-dev";
...
/** 마스킹된 표시용 — 화면에는 원문을 절대 그대로 보여주지 않음 */
